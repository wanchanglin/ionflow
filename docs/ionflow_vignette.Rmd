...
method is *custom*, user must use specific std values such as: 
...
The core part of network and enrichment analysis, clustering, is based on
the symbolic data. 
...
symbolic data: remove genes with all values are zero.
...
terms of genes, i.e. ions are samples/replicates while genes are
variables/features. 
...
```{r dat-pca, fig.cap = "Ion PCA plot"}
expl$plot.pca
```
...
```{r dat-corr, fig.cap = "Ion correlation plots"}
expl$plot.corr
expl$plot.corr.heat
expl$plot.net
```
...
```{r dat-corr-gene, fig.cap = "Correlation plots between ions and genes"}
expl$plot.heat
```

\clearpage

Actually we can perform explanatory analysis using results of gene
clustering:

```{r dat-clus, fig.cap = "Explanotory analysis plots for gene clustering"}
