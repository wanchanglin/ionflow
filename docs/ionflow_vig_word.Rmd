...
description: Ionomics data gene network and enrichment analysis
geometry: margin=1in
fontsize: 11pt
---
...
```{r, tab.cap = 'Processed data summary'}
pre$stats.batch_data
```

The pre-processed data are like this:

```{r, tab.cap = 'Processed data'}
pre$data.gene.zscores %>% head()
```

And the symbolic data are like this:

```{r, tab.cap = 'Symbolic data'}
...
```{r, tab.cap = 'Symbolic data with threshold of 2'}
...
```{r, message = F, tab.cap = 'KEGG enrichment analysis'}
...
```{r, message = F, tab.cap = 'GP Terms enrichment analysis'}
