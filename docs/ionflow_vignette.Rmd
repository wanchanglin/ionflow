...
correction. User can chose not to perform batch correction, otherwise the
user will use either *median* or *median* plus *std* method. If there is
quality control for the batch correction, the user can use it and indicates
in the argument of `control_lines`. Also one argument gives user option how
...
method is *custom*, user uses a specific std file like: 
...
a symbolic data set. This data set is based on the ionomics data and is
...
Note that the symbolic data set is important since the key part of network
and enrichment analysis is based on the hierarchical clustering of symbolic
data. 
...
data and pay more attention to the maximum values. For example, some ions
...
to symbolic data: remove genes with all values are zero.
...
measures on ionomics data are used for constructing network. Before creating
network, these measurement are further filtered by:
...
correleation methods and *cosine* is similar to correlation. For the last
two methods, see publication: [Extraction and Integration of Genetic
Networks from Short-Profile Omic Data
...
For the comparison purpose, we use different similarity methods. Here we
choose *Cosine*:
...
is the universe gene set. The Bioconductor R package
...
Note that there could be none results for KEGG enrichment analysis. Change
arguments such as `min_clust_size` as appropriate.
...
while genes variables/features. The explanatory analysis is normally employed
in the different stages of analysis.
...
The explanatory analysis can be used after gene identification.  Here for
example after gene clustering analysis:

```{r dat-clus, fig.cap = "Explanotory analysis after gene clustering"}
