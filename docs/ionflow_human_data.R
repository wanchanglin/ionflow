...
#' wl-11-12-2020, Fri: use min_clust_size = 8
...
gene_net <- GeneNetwork(data = dat,
                        data_symb = dat_symb,
                        min_clust_size = min, thres_corr = 0.6,
                        method_corr = "pearson")
                        #' method_corr = "cosine")
...
kegg_en <- kegg_enrich(data = dat_symb, min_clust_size = min,
                       pval = 0.05, annot_pkg = "org.Hs.eg.db")
kegg_en

go_en  <- go_enrich(data = dat_symb, min_clust_size = min,
